...
extern const unsigned char *p_wasm;
extern const size_t size_wasm;
...
main(int argc, char *argv[])
{
...
#else
    if (!(wasm_file_buf =
            (uint8 *)bh_copy_wasm_data_to_buffer(p_wasm, size_wasm)))
       goto fail1;
#endif
    ...
    /* load WASM module */
    if (!(wasm_module = wasm_runtime_load(wasm_file_buf, size_wasm,
